...
<div id="login">
        <aside>
            ...
            <figure>
                ...
                <div id="portlet">
                    <h4>SuperAdmin Area</h4>
                    <form class="login-form" action="{{ route('cpannel.master.login') }}" method="POST">
                        @csrf

                <div class="form-group">
                    <span class="input @error('email') is-invalid @enderror">
                    <input class="input_field " type="email" autocomplete="off" name="email" value="{{ $email ?? old('email') }}" required>
