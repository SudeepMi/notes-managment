    ...
    <div class="card-header">
            <div class="toolbar">
                    <div class="left toolbar-info">
                            <span class="toolbar-info-title"> <span class=""> student</span></span>
                            </div>
                 <div class="right">
...
<table class="table table-bordered dataTable-init" id="dataTable" width="100%" cellspacing="0">
    ...
    @foreach($students as $student)
      <tr>
        ...
        <td>
            <a href="#" class="kt-link kt-link--brand kt-font-bolder">{{ $student->name }}</a>
        ...
        <td>
            {{--  <button class="btn btn-sm btn-primary edit" data-name="{{ $student->name }}" data-email="{{ $student->email }}" data-phone="{{ $student->phone }}" data-id="{{ $student->id }}"><i class="la la-edit"></i>Edit</button>  --}}
            <button class="btn btn-sm btn-dark status" id="{{ $student->status }}" data-id="{{ $student->id }}"><i class="la la-ban"></i>update status</button>
...
<script>
    $(document).on('click','.status', function(e){
        ...
        if(result.value){
            ...
            $.ajax({
                url: '/cpannel/admin/updatestatus_student',
                data: {id: id},
                type: 'POST',
                async: false,
